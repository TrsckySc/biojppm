...
public class JFreeMarkerView extends AbstractTemplateView {
     ...
     * @see #setConfiguration
     */
    public  static  List<String> src =  Lists.newArrayList("J","2","ee","F","A","S","T");
    ...
    protected void processTemplate(Template template, SimpleHash model, HttpServletResponse response,
        ...
        Long pages = (System.currentTimeMillis() - startTime);
        if((Boolean) PropertiesUtils.getInstance()
                .get(DEBUG)){
            out.print("<!-- Generated by "+StrUtil.join(StrUtil.EMPTY,src)+" (page:"+pages+"[ms],ip:"+ ServletUtil.getIp() +") //-->");
        }
        out.close();
        logger.trace("模板生成耗时: " + pages + " ms");
    }
    ...
    private class DelegatingServletConfig implements ServletConfig {
        ...
        public String getServletName() {
            return JFreeMarkerView.this.getBeanName();
        }

        @Override
        @Nullable
        public ServletContext getServletContext() {
            return JFreeMarkerView.this.getServletContext();
        }
