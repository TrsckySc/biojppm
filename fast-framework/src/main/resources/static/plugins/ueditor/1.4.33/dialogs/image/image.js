 ...
 *  * j2eefast 修改新增在线图片删除功能 2020-03-30 16:01:20*
 */
...
(function () {
    ...
    OnlineImage.prototype = {
        ...
        pushData: function (list) {
            var i, item, img, icon,delbut, _this = this,
            ...
            for (i = 0; i < list.length; i++) {
                if(list[i] && list[i].url) {
                    ...
                    delbut = document.createElement('span');
                    ...
                    //j2eefast 新增删除功能
                    domUtils.addClass(delbut, 'delbtn fa fa-times-circle-o layui-tab-close fa-search');
                    delbut.setAttribute('name', list[i].url );
                    domUtils.on(delbut,['click'],function () {
                        var del = $(this);
                        try{
                            ...
                            if(window.parent.opt){
                                window.parent.opt.modal.confirm("您确定要删除吗?",function () {
                                    var url = editor.getActionUrl(editor.getOpt('delimageActionName')),
                                        isJsonp = utils.isCrossDomainUrl(url);
                                    ajax.request(url, {
                                        'timeout': 100000,
                                        'dataType': isJsonp ? 'jsonp':'',
                                        ...
                                        'onsuccess': function (r) {
                                            try {
                                                ...
                                                if (json.state == 'SUCCESS') {
                                                    location.reload();
                                                    window.parent.opt.modal.success("删除成功!");
                                                }else{
                                                    if(json.state){
                                                        window.parent.opt.modal.error(json.state);
                                                    }else{
                                                        window.parent.opt.modal.error(json.msg);
                                                    }
                                                    _this.reset();
                                                }
                                            } catch (e) {
                                                window.parent.opt.modal.error("删除失败!");
                                                _this.reset();
                                            }
                                        },
                                        'onerror': function () {
