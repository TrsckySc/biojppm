...
(function($){ 	
    ...
    h.append("<style type='text/css'>  /*.JColResizer{table-layout:fixed;}*/ " +
		".JColResizer > tbody > tr > td, .JColResizer > tbody > tr > th{overflow:hidden;padding-left:0!important; padding-right:0!important;}" +
		"  .JCLRgrips{ height:0px; position:relative;} .JCLRgrip{margin-left:-5px; position:absolute; z-index:5; } " +
		...
		".JCLRLastGrip{position:absolute; width:1px; } .JCLRgripDrag{ border-left:1px dotted black;	} .JCLRFlex{width:auto!important;} " +
		".JCLRgrip.JCLRdisabledGrip .JColResizer{cursor:default; display:none;}</style>");
	...
	var init = function( tb, options){
		...
		if(_ColResizable == 0 )if (t.opt.hoverCursor !== 'e-resize') h.append("<style type='text/css'>.JCLRgrip .JColResizer:hover{cursor:"+ t.opt.hoverCursor +"!important}</style>");  //if hoverCursor has been set, append the style
		//J2eeFAST 优化表格列拖动自动显示表格边框
		if(_ColResizable == 0 && options.table) h.append("<style type='text/css'>.fixed-table-container {border-left: 1px solid #e7eaec!important;border-top: 1px solid #e7eaec!important;} .table-striped .table, .table-striped .table, .table>thead>tr>th, .table>tbody>tr>th, .table>tfoot>tr>th, .table>thead>tr>td, .table>tbody>tr>td, .table>tfoot>tr>td {border-right: 1px solid #e7eaec!important;}</style>");
		_ColResizable ++;
		t.addClass(SIGNATURE).attr(ID, id).before('<div class="JCLRgrips"/>');	//the grips container object is added. Signature class forces table rendering in fixed-layout mode to prevent column's min-width
		t.g = []; t.c = []; t.w = t.innerWidth(); t.gc = t.prev(); t.f=t.opt.fixed;	//t.c and t.g are arrays of columns and grips respectively
