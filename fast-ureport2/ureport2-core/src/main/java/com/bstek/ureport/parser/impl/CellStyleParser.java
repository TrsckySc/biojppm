...
public class CellStyleParser implements Parser<CellStyle> {
	...
	public CellStyle parse(Element element) {
		...
		//是否自动换行
		String wordWrap =element.attributeValue("word-wrap");
		if(StringUtils.isNotBlank(wordWrap)){
			style.setWordWrap(Boolean.valueOf(wordWrap));
		}
		...
		//删除线
		String delline=element.attributeValue("delline");
		if(StringUtils.isNotBlank(delline)){
			style.setDelline(Boolean.valueOf(delline));
		}
		...
		String htmlEscape = element.attributeValue("html-escape");
		if(StringUtils.isNotBlank(htmlEscape)){
			style.setHtmlEscape(Boolean.valueOf(htmlEscape));
		}
