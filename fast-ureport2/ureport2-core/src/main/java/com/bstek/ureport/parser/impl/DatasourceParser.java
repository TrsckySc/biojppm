	...
	private List<DatasetDefinition> parseDatasets(Element element){
		...
		for(Object obj:element.elements()){
			...
			if(type.equals("sql")){
				...
				//设置分页
				String ispage= ele.attributeValue("ispage");
				if(StringUtils.isNotBlank(ispage)){
					dataset.setPage(Boolean.valueOf(ispage));
				}
				String pageSize= ele.attributeValue("pagesize");
				if(StringUtils.isNotBlank(pageSize)){
					dataset.setPageSize(Integer.valueOf(pageSize));
				}
